...
main(int argc, char** argv)
{
  ...
  nh_private.getParam("VoxelWidth", scale);//读取yaml文件保证与planner一致
  nh_private.getParam("MapBound", mapBound);

  sizeX=floor(min(abs(mapBound[0]),abs(mapBound[1])));
  sizeY=floor(min(abs(mapBound[2]),abs(mapBound[3])));
  sizeZ=floor(mapBound[5]);
  std::cout<<"[in mockmap sizeX]"<<sizeX<<std::endl;
  std::cout<<"[in mockmap sizeY]"<<sizeY<<std::endl;
  std::cout<<"[in mockmap sizeZ]"<<sizeZ<<std::endl;

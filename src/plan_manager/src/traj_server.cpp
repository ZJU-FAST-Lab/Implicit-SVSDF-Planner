...
std::shared_ptr<Trajectory<7>> snap_traj;
std::shared_ptr<Trajectory<5>> jerk_traj;
...
void cmdCallback(const ros::TimerEvent &e)
{
  ...
  {
    pos = jerk_traj->getPos(t_cur);
    vel = jerk_traj->getVel(t_cur);
    acc = jerk_traj->getAcc(t_cur);
    jer = jerk_traj->getJer(t_cur);

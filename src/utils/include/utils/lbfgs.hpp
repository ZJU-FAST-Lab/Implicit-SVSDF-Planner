...
namespace lbfgs
{
    ...
    struct lbfgs_parameter_t
    {
        ...
        /**
         * 是否使用tolerance 在 line search
        */
       bool with_tole_ls = false;

        /**
         * Super Case ,针对t*优化问题定制的变量
         * 
        */
       bool cast_x_range = false;
       double x_min = 0.0;
       double x_max = 0.0;
    };
    ...
    enum
    {
        ...
        LBFGSERR_MAXIMUMLINESEARCH, //-1009
                                        ...
                                        const lbfgs_parameter_t &param)
    {
        ...
        double finit, dginit, dgtest, dstest, fpos_init;
        double mu = 0.0, nu = stpmax;

        int tol_sw = (param.with_tole_ls == true) ? 1 : 0;
        ...
        bool cast_xr = param.cast_x_range;
        double x_min = param.x_min;
        double x_max = param.x_max;
        while (true)
        {
            x = xp + stp * s;

            /* Evaluate the function and gradient values. */
            f = cd.proc_evaluate(cd.instance, x, g, f_cost);
            ++count;

            // std::cout<<"stp = "<<(stp * s).norm()<<std::endl;
            // if((nu - mu) < 1e-4){

                // Eigen::VectorXd gg = g;
                // Eigen::VectorXd num_g = (cd.proc_evaluate(cd.instance, x, gg) - cd.proc_evaluate(cd.instance, xp, gg)) * ((x - xp).cwiseInverse());
                // std::cout<<"dg = "<<(num_g - g).norm()<<std::endl;
            // }
            double stp_conv1 = (1.0 - stp) > 0.0 ? (0.1 * (1.0 - stp)) : 0.0;
            ...
            if( cast_xr && ( x(0) > x_max || x(0) < x_min )){
                nu = stp;
                brackt = true;
            }
            /* Check the Armijo condition. */
            else if (f > finit + stp * dgtest + 0 * tol_sw * (stp_conv1 * finit) )
            {
                nu = stp;
                brackt = true;
                // cond1 ++;
            ...
            else
            {
                ...
                // std::cout<<"g = "<<g.dot(s) << " || " << dstest<<std::endl;
                if (0)
                // if (g.dot(s) < dstest)
                {
            ...
            if (param.max_linesearch <= count)
            {
                ...
                // std::cout<<"c1 = "<<cond1 <<"\tc2 = "<<cond2<<std::endl;
                              ...
                              const lbfgs_parameter_t &param)
    {
        ...
        double p_cost,p_cost_new;
        /* Evaluate the function value and its gradient. */
        fx = cd.proc_evaluate(cd.instance, x, g, p_cost);
        ...
        else
        {
            ...
            */
            ...
            while (true)
            {
                ...
                ls = line_search_lewisoverton(x, fx,p_cost,  g, step, d, xp, gp, step_min, step_max, cd, param);
                ...
                */
                ...
                if (ys > cau)
                {
                    ...
                    */
                    ...
                    end = (end + 1) % m;//end取0,1,2....m-1

                    j = end;
                    // std::cout<<"end:"<<end<<" j:"<<j<<std::endl;
                    for (i = 0; i < bound; ++i)
                    {
                        ...
                        // std::cout<<""
                        // std::cout<<"alpha"<<" "<<lm_alpha(j)<<std::endl;
                        /* q_{i} = q_{i+1} - \alpha_{i} y_{i}. */
                        d += (-lm_alpha(j)) * lm_y.col(j);
                    }

                    d *= ys / yy;
                    // std::cout<<"ys/yy: "<<ys / yy<<std::endl;
                    for (i = 0; i < bound; ++i)
                    {
                        ...
                        // std::cout<<"delta2 "<<"i:"<<i<<" "<<(lm_alpha(j) - beta) * lm_s.col(j).transpose().head(4)<<std::endl;
                        j = (j + 1) % m; /* if (++j == m) j = 0; */
                    }
                    if(d.norm()>=0.04||d.dot(g)>0)//打补丁
                    {
                    double fx_ = cd.proc_evaluate(cd.instance, x, g_recover, p_cost_new);
                    d=-g_recover;
